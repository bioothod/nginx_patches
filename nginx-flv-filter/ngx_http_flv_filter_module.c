...
static ngx_int_t ngx_http_flv_parse(ngx_http_request_t * r, ngx_http_flv_filter_ctx_t * ctx);
...
static ngx_int_t ngx_http_flv_header_filter(ngx_http_request_t * r)
{
	...
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "flv_filter: header: type: %d", ffcf->type);
...
static ngx_int_t ngx_http_flv_parse(ngx_http_request_t * r, ngx_http_flv_filter_ctx_t * ctx)
{
	...
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
		       "flv_filter: body: parse: content_length: %d, args_len: %d",
		       content_length, r->args.len);

	if (r->args.len) {
		err = ngx_http_arg(r, (u_char *) "start", 5, &value);
		if (err != NGX_OK) {
			return NGX_DECLINED;
		}
...
static ngx_int_t ngx_http_flv_body_filter(ngx_http_request_t * r, ngx_chain_t * in)
{
	ngx_http_flv_filter_ctx_t *ctx;
	off_t start, last;
	ngx_buf_t *buf;
	...
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "flv_filter: body: type: %d", ffcf->type);
	...
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
		       "flv_filter: body: ctx: offset: %d, start: %d",
	...
	for (cl = in; cl; cl = cl->next) {
		...
		ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
			       "flv_filter: body: buf: %O-%O", start, last);
...
static ngx_int_t ngx_http_flv_filter_init(ngx_conf_t * cf)
{
	...
	ngx_conf_log_error(NGX_LOG_INFO, cf, 0, "flv_filter: init");
	return NGX_OK;
}
